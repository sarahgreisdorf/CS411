...
exports.pay_order = async function(req, res) {
  ...
  //Query DB to find open orders
	...
	try {
		const toPay = await Order.findOne({"state": "OPEN"}, function(order, error) {
			//If order is null, no open orders in db
    ...
    await Order.updateOne({"state": "OPEN"}, {"state": "COMPLETED"})
		...
		//Returns error when there are no open orders in the db
		res.status(500).send('No open orders')
	}
}